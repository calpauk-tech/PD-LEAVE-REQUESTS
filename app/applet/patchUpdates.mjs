...
if (match) {
    ...
    // If exact formatting fails, try to replace it separately
    console.log("Could not find billing mode cell block");
}
...
const pillSelectorLogicOld = /className=\{\`w-28 text-xs font-medium border rounded px-1 py-1 focus:outline-none focus:ring-1 \$\{[\s\S]*?bg-red-100 border-red-200 text-red-800 focus:ring-red-500'\s*\}\`\}/;
...
const pillSelectColoring = `className={\`w-28 text-xs font-bold border-0 rounded px-2 py-1.5 focus:outline-none focus:ring-2 cursor-pointer shadow-sm \${
                    (adj.requestStatus || 'Requested') === 'Requested' ? 'bg-yellow-200 text-yellow-900 focus:ring-yellow-400' :
                    (adj.requestStatus || 'Requested') === 'Approved' ? 'bg-green-200 text-green-900 focus:ring-green-400' :
                    'bg-red-200 text-red-900 focus:ring-red-400'
                }\`}`;
